...
static int enable_ext_sig_to_client[MAX_NET_CLIENTS];
					/* bbs2 extension (non-standard): when set, monitor frames sent to */
	...
	switch (fromto) {
	  ...
	  case FROM_CLIENT:
	    ...
	    switch (pmsg->datakind) {
	      ...
	      case 'q': strlcpy (datakind, "Enable Extended Signal Quality (bbs2)",	sizeof(datakind)); break;
	...
	while (1) {
	  ...
	  while (1) {
	    ...
	    if (send_queue_head[client] == NULL)
	      send_queue_tail[client] = NULL;
	    send_queue_len[client]--;
	    fd = client_sock[client];	/* save under lock to avoid fd-recycling race */
	    pthread_mutex_unlock (&send_queue_mutex[client]);
 ...
 */
	...
	for (int client=0; client<MAX_NET_CLIENTS; client++) {
	  if (enable_send_monitor_to_client[client] && client_sock[client] > 0) {
	    ...
	    // bbs2 extension (non-standard, opt-in via 'q'): stash per-frame signal
 ...
 */
	  ...
	  switch (cmd.hdr.datakind) {
	    ...
	    case 'q':				/* bbs2 extension: enable extended signal quality reporting */
