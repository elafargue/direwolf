...
Fork-local extension (branch `agwpe-signal-quality`, layered on `udp-audio-out`).
Consumer + full rationale: bbs2 `bbs/plugins/heard/SIGNAL_QUALITY_SPEC.md`.
...
## Design constraint: backwards compatibility

A stock AGWPE client (Xastir, APRSIS32, UI-View, YAAC, …) connected to a patched
Direwolf **must see byte-identical frames**. Guaranteed by making the extension
**per-client opt-in**: the extra bytes are written only in the copy sent to a
client that explicitly enabled it. Everyone else gets today's exact behavior.
